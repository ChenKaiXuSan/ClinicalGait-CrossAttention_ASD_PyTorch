...
\begin{highlights}
\item PoseGated fuses gait RGB video with doctor-annotated attention maps by
      channel-wise gating.
\item $88.3\%$ 3-class accuracy vs.\ $78.7\%$ RGB-only baseline on
      patient-grouped 3-fold CV.
\item Ablations show shallow fusion is best; fusing at all stages is worst.
\item Auxiliary bg/temporal losses and RGB-biased gate init reduce accuracy.
\item Attention localises the clinical ROI on held-out patients (pointing game
      $1.00$).
\end{highlights}
