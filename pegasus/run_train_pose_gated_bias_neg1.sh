...
#PBS -N pose_gated_bias_neg1_train    # 🏷 Ablation A2c: gate_init_bias = -1.0
...
echo "Ablation A2c: gate_init_bias=-1.0, full multi [0..4]"
...
python -m project.train data.root_path=${root_path} \
    ...
    train.gpu=1 \
    train.experiment=pose_gated_biasneg1_f${fold} \
    data.num_workers=$(( $(nproc) / 3 ))


# Experiment notes
###############################################################################
# 实验编号: A2c — gate init bias 消融 (初期偏向 pose 分支)
# 对照: A2a = pose_gated_full (bias=2.0)，A2b = bias0 (bias=0.0)
###############################################################################
