...
#PBS -N se_atn_multi_train                     # 🏷 作业名称
#PBS -t 0-4                     # 🗂 作业数组（可选，0-4 表示 5 个任务）
#PBS -o logs/pegasus/train_se_atn_multi_out_fusion_${PBS_SUBREQNO}.log            # 📤 标准输出日志
#PBS -e logs/pegasus/train_se_atn_multi_err_fusion_${PBS_SUBREQNO}.log            # ❌ 错误输出日志
...
# === 下载预训练模型（如果需要） ===
# wget -O /work/SKIING/chenkaixu/code/ClinicalGait-CrossAttention_ASD_PyTorch/checkpoints/SLOW_8x8_R50.pyth https://dl.fbaipublicfiles.com/pytorchvideo/model_zoo/kinetics/SLOW_8x8_R50.pyth
...
python -m project.main data.root_path=${root_path} model.fuse_method=se_atn train.fold=5 data.num_workers=$((NUM_WORKERS / 3)) model.fusion_layers=${PBS_SUBREQNO} model.ablation_study=multi