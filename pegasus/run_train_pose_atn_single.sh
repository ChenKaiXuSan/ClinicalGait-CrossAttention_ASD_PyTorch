...
python -m project.main data.root_path=${root_path} model.fuse_method=pose_atn train.fold=10 data.num_workers=$((NUM_WORKERS / 3)) model.fusion_layers=${PBS_SUBREQNO} model.ablation_study=single