...
echo "Ablation A4a: loss_selection=[cls,attn_loss,tmp] → bg_loss removed"
...
python -m project.train data.root_path=${root_path} \
    ...
    train.gpu=1 \
    train.experiment=pose_gated_nobg_f${fold} \
    data.num_workers=$(( $(nproc) / 3 ))


# Experiment notes
###############################################################################
# 实验编号: A4a — 损失消融: 去掉 background suppression loss (w_bg 置 0)
# 其余同 pose_gated_full。
###############################################################################
